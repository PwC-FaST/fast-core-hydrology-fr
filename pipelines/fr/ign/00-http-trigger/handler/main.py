...
def handler(context, event):
    ...
    try:
        ...
        archive_regex = "BDTOPO_2-2_HYDROGRAPHIE_SHP_.*_{1}_{0}-[0-9]{{2}}-[0-9]{{2}}.7z(.001)?$".format(year,region)

        # BD TOPO® Hydrographie / IGN
        hydro_url = FunctionConfig.hydro_fr_ign_url
        context.logger.info("BD TOPO® hydrographie: {0}".format(hydro_url))

        r = requests.get(hydro_url)

        # parse response content
        bs = BeautifulSoup(r.content,'html.parser')
        hrefs = bs.findAll("a", {"class": "fleche2"})

        # search the requested archive
        links = []
        for h in hrefs:
            if h.get('href') and re.search(archive_regex,h.get('href')):
                links.append(h.get('href'))

        # check if there is indeed an archive to download
        if not len(links) == 1:
            context.logger.warn_with("Failed to scrape French BD TOPO Hydrography archive for year '{0}' and region '{1}' (not found), regex={2}"
                .format(year,region,archive_regex))
            raise NuclioResponseError(
                "Failed to scrape French BD TOPO Hydrography archive for year '{0}' and region '{1}' (not found)".format(year,region),requests.codes.bad)
        ...
        context.logger.info("French BD TOPO Hydrography archive for year '{0}' and region '{1}' successfully scraped".format(year,region))
...
class FunctionConfig(object):
    ...
    hydro_fr_ign_url = None

    normalized_hydro_properties = dict([
        ('name',{'sourceProp': 'NOM', 'defaultOrigValueToDelete': 'NR'})])
...
class Helpers(object):
    ...
    def load_configs():
        ...
        FunctionConfig.hydro_fr_ign_url = \
            os.getenv('HYDRO_FR_IGN_URL','http://professionnels.ign.fr/bdtopo-hydrographie')
    ...
    def parse_body(context, body):
        ...
        if year != 2018:
            context.logger.warn_with('Only archives for year 2018 are currently available !')
            raise NuclioResponseError(
                'Only archives for year 2018 are currently available !',requests.codes.bad)
        ...
        if not len(body['region']) == 3:
            context.logger.warn_with("'region' attribute must be a trigram (ex: FXX, GLP, MTQ, GUF) !")
            raise NuclioResponseError(
                "'region' attribute must be a trigram (ex: FXX, GLP, MTQ, GUF) !",requests.codes.bad)
        region = body['region']
